...
<html>
	...
	<?php include_once('./includes/bodyUpper.php') ?>
		<div class='content'>
			...
			<a href='./activityAssistantPharmacist.php'>
				<button class='orange button'>Go Back to Assistant Pharmacists' Log</button>
			</a>
...
$activityResult = mysqli_query($connect, "SELECT * FROM activitytable WHERE userType='Assistant Pharmacist' ORDER BY activityDate DESC");
...
if($aCount != 0){
	...
	for($m=0; $m<count($queryArr); $m++){
		...
		while($postData = mysqli_fetch_array($queryArr[$m])){
			if($minicount == 0){
				$act2 = mysqli_query($connect, "SELECT * FROM activitytable WHERE userType='Assistant Pharmacist' ORDER BY activityDate DESC");
				if($detArr[$m] == "issued"){
