	...
	/* COMPUTES AVERAGE CONSUMPTION */
		...
		while($row1 = mysqli_fetch_array($productId)) {
			...
			$sql = 
				...
				WHERE vendorordertable.productId = '" . $row1['productId'] . "'";
			$deliveryTime = mysqli_query($connect, $sql);
			$row5 = mysqli_fetch_array($deliveryTime);
				...
				if($date != $row2['date']) { 
					...
					while($row3 = mysqli_fetch_array($aveCons)) {
						while($row4 = mysqli_fetch_array($consumed)) {
							...
							//update reorder level in dynamic product table
							$reorderLevel = $row5['deliveryTime'] * $averageConsumptionToday;
							$sql = 
