package Exercise05_PriorityWithoutPreemption;
...
public class PWOP_Tuazon {
	...
	public static void execute(ArrayList<Process> list) {
		...
		for(int i = 1; i < list.size(); i++){
			...
			} else if(firstProcess.getArrivalTime() == list.get(i).getArrivalTime() && firstProcess.getPriorityNum() > list.get(i).getPriorityNum()){
				firstProcess = list.get(i);
			}
		}
		...
		while(finalList.size() != list.size()){
			...
			for(int i = 1; i < tempList.size(); i++){
				if(nextProcess.getPriorityNum() > tempList.get(i).getPriorityNum()){
	...
	private static void InterfaceCreator(ArrayList<Process> list, String string, String string2, double throughput){
		...
		mainframe.setResizable(false);
			...
			JPanel mainpanel = new JPanel(new GridBagLayout());
				...
				JLabel label = new JLabel("Non Preemptive Priority", SwingConstants.CENTER);
				...
				label = new JLabel("Priority", SwingConstants.CENTER);
					c.gridx = 4;
				mainpanel.add(label, c);
				...
				for(int i  = 0; i < list.size(); i++){
					...
					label = new JLabel(String.valueOf(list.get(i).getBurstTime()), SwingConstants.CENTER);
						c.gridx = 3;
					mainpanel.add(label,c);
					label = new JLabel(String.valueOf(list.get(i).getPriorityNum()), SwingConstants.CENTER);
						c.gridx = 4;
					mainpanel.add(label,c);
