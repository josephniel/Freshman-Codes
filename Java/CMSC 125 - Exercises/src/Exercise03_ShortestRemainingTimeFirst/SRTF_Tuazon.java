...
public class SRTF_Tuazon {

	public static void execute(ArrayList<Process> list) {
		
		int finalEndTime = getFinalEndTime(list);
		
		int[] ganttChart = new int[finalEndTime];
	...
	private static void InterfaceCreator(ArrayList<Process> list, String string, String string2, double throughput){
		...
		mainframe.setResizable(false);
			...
			JPanel mainpanel = new JPanel(new GridBagLayout());
				...
				JLabel label = new JLabel("Shortest Remaining Time First", SwingConstants.CENTER);
	...
	private static int getFinalEndTime(ArrayList<Process> list) {
		...
		return finalList.get(finalList.size()-1).getEndTime();
